...
class AdminProductsiteController extends AppController {
	...
	public function indexredirect() {
		$this->render('/Admin/Product/redirect');
	}

	public function index() {
		try {
			if(!isset($_SESSION['Auth']['User']['TKaisya']['kaisyanm'])) {
				// 初期表示
				$this->redirect ( ['controller' => 'Error','action' => 'systemError' ] );
				// リクエストデータが空白以外の場合
			} else {
				$this->set('kaisyanm',$_SESSION['Auth']['User']['TKaisya']['kaisyanm']);
				$query = $this->TKaisya->find('all', array(
								'joins' => array(
									...
									array(
											'table' => sprintf("(SELECT GROUP_CONCAT(kaiinnm ORDER BY kaiinsbcd ASC,kaiinnm ASC) as kaiinnmGC,t_kaiin.* FROM t_kaiin GROUP BY kaisyacd)"),
											'alias' => 'tkn',
											'type' => 'LEFT',
								...
								'conditions' => array('TKaisya.kaisyacd' => $_SESSION['Auth']['User']['TKaisya']['kaisyacd']),
				...
				// for condition on TPrkeiyaku g_keiyaku_to date
				$curentdate_without_time = date("Y-m-d", strtotime($this->Common->getSystemDateTime ()));
					...
					if($value ['TKaisya']['kaisyacd']!="") {
						$gettprkeiyaku = $this->TPrkeiyaku->find('first', array(
												...
												'conditions' => array('kaisyacd' => $value ['TKaisya']['kaisyacd'],
																	'g_keiyaku_to >=' => $curentdate_without_time,
												)));
						...
						if(isset($gettprkeiyakueditfrom[0]['g_keiyaku_from'])) {
							$query[$key]['TPrkeiyaku']['g_keiyaku_from'] = $gettprkeiyakueditfrom[0]['g_keiyaku_from'];
						}
						...
						if(isset($gettprkeiyakueditto[0]['g_keiyaku_to'])) {
							$query[$key]['TPrkeiyaku']['g_keiyaku_to'] = $gettprkeiyakueditto[0]['g_keiyaku_to'];
						}
					}
				}
	...
	public function add() {
		...
		try {
			if(empty($this->request->data)){
				...
				/*$gettprkeiyaku1 = $this->TPrkeiyaku->find('first', array(
											'fields' => array('g_keiyaku_from','g_keiyaku_to'),
											'conditions' => array('TPrkeiyaku.g_keiyaku_to > CURDATE()',
																'kaisyacd' => $this->request->data['Productaddfrm']['kaisyacd'])));
				$gettprkeiyaku2 = $this->TPrkeiyaku->find('first', array(
											'fields' => array('g_keiyaku_from','g_keiyaku_to'),
											'conditions' => array('TPrkeiyaku.g_keiyaku_to < CURDATE()',
																'kaisyacd' => $this->request->data['Productaddfrm']['kaisyacd']),
											'order'=> array('g_keiyaku_to' => 'DESC')));
				if(!empty($gettprkeiyaku1['TPrkeiyaku']['g_keiyaku_from'])) {
					$this->set('gettprkeiyaku', $gettprkeiyaku1);
				} else {
					$this->set('gettprkeiyaku', $gettprkeiyaku2);
				}*/
	...
	public function edit() {
		...
		try {
			if(empty($this->request->data)){
				...
				/*$gettprkeiyaku1 = $this->TPrkeiyaku->find('first', array(
											'fields' => array('g_keiyaku_from','g_keiyaku_to'),
											'conditions' => array('TPrkeiyaku.g_keiyaku_to > CURDATE()','kaisyacd' => $this->request->data['Producteditfrm']['kaisyacd'])));
				$gettprkeiyaku2 = $this->TPrkeiyaku->find('first', array(
											'fields' => array('g_keiyaku_from','g_keiyaku_to'),
											'conditions' => array('TPrkeiyaku.g_keiyaku_to < CURDATE()','kaisyacd' => $this->request->data['Producteditfrm']['kaisyacd']),
											'order'=> array('g_keiyaku_to' => 'DESC')));
