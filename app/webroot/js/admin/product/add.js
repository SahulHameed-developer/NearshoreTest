$(document).ready(function() {
	...
	$(".b-preview").click(function () {
		$('#previewflg').val('1');
		$('#adminPRshohinregister').attr('target', '_blank');
		// $('#adminPRshohinregister').attr('action', PR_PREVIEW_PATH);
		$('#adminPRshohinregister').attr('action', BASE_PATH+PR_PREVIEW_PATH);
		$('#adminPRshohinregister').submit();
	});
	$(".add").click(function(){
		$('#adminPRshohinregister').submit();
	});
	...
	$('#prtantou').change(function() {
		var validator = $("#adminPRshohinregister").validate();
		validator.resetForm();
		if($(this).val() != "") {
			var id = this.id;
			$.ajax({
...
(function($,W,D)
{
	...
	jqueryValidation.UTIL =
	{
		...
		{
			// 入会申込フォーム確認
			$("#adminPRshohinregister").validate({
				onkeyup: false,
							...
							buttons: {
								OK: {
									action: function(){
										...
										$('#adminPRshohinregister').attr('target', '');
										$('#adminPRshohinregister').attr('action', '../AdminProductsite/register');
										var data = new FormData();
										var serializedData = $("#adminPRshohinregister").serialize();
