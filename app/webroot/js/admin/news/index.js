$(document).ready(function() {
	// お知らせ編集。
	$(".edit").click(function () {
		$("#adminNewseditId").val($(this).attr("data-id"));
		$("#adminNewseditSyasinKey").val($(this).attr("data-syasinkey"));
		$( "#adminNewsedit" ).submit();
	});
...
function sample() {
	var Todate = $("#oshiraseDtTo").val();
	var Fromdate = $("#oshiraseDtFrm").val();
	alert(Todate);
...
(function($,W,D)
{
	...
	jqueryValidation.UTIL =
	{
		...
		{
			// 入会申込フォーム確認
			$("#oshiraseSearchForm").validate({
				rules: {
					// 期間（From）
					oshiraseDtFrm: "dateFormat",
					// 期間（To）
					oshiraseDtTo: "dateFormat"
				},
				messages: {
					//期間（From）のエラーメッセージ。
					oshiraseDtFrm: "正しい年月を入力してください。",
					// 期間（To）のエラーメッセージ。
					oshiraseDtTo: "正しい年月を入力してください。"
				},
