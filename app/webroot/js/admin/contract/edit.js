$(document).ready(function() {
	var dt = new Date();
	var curyearmonth = dt.getFullYear() + "/" + String("0" + (dt.getMonth() + 1)).slice(-2);
	var to = $("#g_keiyaku_to").val();
	if ($("#kykbn").val() != "1") {
		$('#s_keiyaku_from').attr('disabled', 'disabled');
		$("#s_keiyaku_from").removeClass('datepickerfmt');
		$('#s_keiyaku_to').attr('disabled', 'disabled');
		$("#s_keiyaku_to").removeClass('datepickerfmt');
		$('#s_keikin').attr('disabled', 'disabled');
	}
	if ($("#kykbn").val() == "9" || curyearmonth > to) {
		$('#s_keiyaku_from').attr('disabled', 'disabled');
		$("#s_keiyaku_from").removeClass('datepickerfmt');
		$('#s_keiyaku_to').attr('disabled', 'disabled');
		$("#s_keiyaku_to").removeClass('datepickerfmt');
		...
		$('#g_keiyaku_from').attr('disabled', 'disabled');
		$("#g_keiyaku_from").removeClass('datepickerfmt');
		$('#g_keiyaku_to').attr('disabled', 'disabled');
		$("#g_keiyaku_to").removeClass('datepickerfmt');
		$('#g_keikin').attr('disabled', 'disabled');
	}
	...
	if($("#g_keiyaku_from").prop('disabled') == false) {
		...
		$('#g_keiyaku_from, #g_keiyaku_to').MonthPicker()
		    .on("input change click", function (e) {
			...
			$("#ContractUpdatefrm").validate({
				...
				errorPlacement: function(error, element) {
					...
					if(element.attr("name") == "s_keiyaku_from" || element.attr("name") == "s_keiyaku_to") {
						$(".s_date_errorft").html("");
						error.appendTo('.s_date_errorft');
					} else if(element.attr("name") == "g_keiyaku_from" || element.attr("name") == "g_keiyaku_to") {
						$(".g_date_errorft").html("");
						error.appendTo('.g_date_errorft');
					} else {
				...
				messages: {
					...
					s_keiyaku_from: {
						...
						prevaltochck: "前の契約期間と重複する期間の指定は出来ません。",
						nxtvalfrmchck: "次の契約期間と重複する期間の指定は出来ません。"
					},
					s_keiyaku_to: {
						required:"初回契約期間（終了月）を入力してください。",
						monthFormat: "初回契約期間（終了月）が不正な形式です。",
						slessThan:"期間の開始、終了を正しく入力してください。",
						smonthcount:"契約期間と契約月数が異なっています。契約期間を修正してください。",
						skeiyakukikan: "初回契約期間（終了月）は 初回契約期間（開始月) 以降の日付を指定してください。",
						prevaltochck: "前の契約期間と重複する期間の指定は出来ません。",
						nxtvalfrmchck: "次の契約期間と重複する期間の指定は出来ません。"
					},
					...
					g_keiyaku_from: {
						required:"契約期間（開始月）を入力してください。",
						monthFormat: "契約期間（開始月）が不正な形式です。",
						gkeiyakukikan: "契約期間（終了月）は 契約期間（開始月) 以降の日付を指定してください。",
						prevaltochck: "前の契約期間と重複する期間の指定は出来ません。",
						nxtvalfrmchck: "次の契約期間と重複する期間の指定は出来ません。"
					},
					g_keiyaku_to: {
						required:"契約期間（終了月）を入力してください。",
						monthFormat: "契約期間（終了月）が不正な形式です。",
						glessThan:"期間の開始、終了を正しく入力してください。",
						gmonthcount:"契約期間と契約月数が異なっています。契約期間を修正してください。",
						gkeiyakukikan: "契約期間（終了月）は 契約期間（開始月) 以降の日付を指定してください。",
						prevaltochck: "前の契約期間と重複する期間の指定は出来ません。",
						nxtvalfrmchck: "次の契約期間と重複する期間の指定は出来ません。"
					},
					g_keikin: { 
						required:"契約金額を入力してください。"
					}
				},
				submitHandler: function(form) {
					$.confirm({
						...
						buttons: {
							OK: {
								action: function(){
									$('#s_keiyaku_from').attr('disabled', false);
									$("#s_keiyaku_from").addClass('datepickerfmt');
									$('#s_keiyaku_to').attr('disabled', false);
									$("#s_keiyaku_to").addClass('datepickerfmt');
									$('#s_keikin').attr('disabled', false);
									$('#g_keiyaku_from').attr('disabled', false);
									$("#g_keiyaku_from").addClass('datepickerfmt');
									$('#g_keiyaku_to').attr('disabled', false);
									$("#g_keiyaku_to").addClass('datepickerfmt');
									$('#g_keikin').attr('disabled', false);
									$("#updatecls").addClass('updatecls');
				                	$('.b-release').prop('disabled', true);
									$('#ContractUpdatefrm').attr('target', '');
									$('#ContractUpdatefrm').attr('action', '../AdminContract/update');
									var data = new FormData();
									var serializedData = $("#ContractUpdatefrm").serialize();
...
jQuery.validator.addMethod("smonthcount", function(value, element) {
...
jQuery.validator.addMethod("gmonthcount", function(value, element) {
	var monthscheck = $( 'input[name=ktukisuu]:checked' ).val();
	var monthval;
	if(monthscheck == 1) {
		monthval = 6;
	...
	var g_keiyaku_from = new Date($("#g_keiyaku_from").val()+"/01");
	var g_keiyaku_to = new Date($("#g_keiyaku_to").val()+"/01");
	var difference = (g_keiyaku_to.getFullYear()*12 + g_keiyaku_to.getMonth()) - (g_keiyaku_from.getFullYear()*12 + g_keiyaku_from.getMonth());
	if(monthval > difference) {
		return true;
	} else {
		return false;
...
jQuery.validator.addMethod("prevaltochck", function(value, element) {
	var to = $("#prevaluesto").val();
	if(value != "") {
		if(to < value) {
			return true;
		} else {
...
jQuery.validator.addMethod("nxtvalfrmchck", function(value, element) {
	var from = $("#nextvaluesfrom").val();
	if(value != "") {
		if(from > value) {
			return true;
		} else {
