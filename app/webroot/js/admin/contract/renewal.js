$(document).ready(function() {
	...
	var lastgfrom = $("#g_keiyaku_from_hidden").val();
	var lastgto = $("#g_keiyaku_to_hidden").val();
	var dt = new Date();
	var curyearmonth = dt.getFullYear() + "/" + String("0" + (dt.getMonth() + 1)).slice(-2);
	if((lastgfrom <= curyearmonth) && (curyearmonth <= lastgto)) {
	} else {
		$("#g_keiyaku_from").val('');
		$("#g_keiyaku_to").val('');
		$("#g_keikin").val('');
	...
	$(".renewal").click(function(){
		$('#ContractrenewalUpdatefrm').submit();
	});
...
(function($,W,D)
{
	...
	jqueryValidation.UTIL =
	{
		...
		{
			// 入会申込フォーム確認
			$("#ContractrenewalUpdatefrm").validate({
				onkeyup: false,
				...
				messages: {
					...
					g_keiyaku_from: {
						...
						lastgkeiyakukikan: "過去の契約期間と重複する期間の指定は出来ません。",
						...
						buttons: {
							OK: {
								action: function(){
									...
									$('#ContractrenewalUpdatefrm').attr('target', '');
									$('#ContractrenewalUpdatefrm').attr('action', '../AdminContract/renewalupdate');
									var data = new FormData();
									var serializedData = $("#ContractrenewalUpdatefrm").serialize();
									serializedData = decodeURIComponent(serializedData);
									data.append('otherFields', serializedData);
									var xhr = new XMLHttpRequest();
								 	xhr.open('POST', 'renewalupdate', true);
								 	xhr.send(data);
								 	xhr.onload = function () {
...
jQuery.validator.addMethod("lastgkeiyakukikan", function(value, element) {
	var lastgfrom = $("#g_keiyaku_from_hidden").val();
	var lastgto = $("#g_keiyaku_to_hidden").val();
	var gfrom = $("#g_keiyaku_from").val();
	var gto = $("#g_keiyaku_to").val();
	if(gto != "") {
		if(lastgto >= gfrom || lastgto >= gto) {
			return false;
		}else {
			return true;
		}
