$(document).ready(function() {	
	...
	$("#iinkaicd").on("focus",function () {
        previous = $("#iinkaicd").val(); 
    });

	$("#iinkaicd").change(function () {
        current = $("#iinkaicd").val(); 
        if(current!="") {
	...
	$(".b-preview").click(function () {
		// alert('未作成');
		$('#previewflg').val('1');
		$('#frmAdminCommittee').submit();
	}); 
	$(".back-preview").click(function () {
		$('#frmCommittee').attr('action', '../Committee/index');
		$('#frmCommittee').submit();
	}); 
	$(".buttonsearch").click(function () {
		$('#newRegister').text('更新');
		$('#previewflg').val('0');
		$('#frmAdminCommitteesearch').attr('action', 'search');
		$('#frmAdminCommitteesearch').submit();
	});
		...
		setupFormValidation: function()
		{
			...
			$('#frmAdminCommittee').validate({
				...
				submitHandler: function(form) {
					...
					if($('#previewflg').val() == 1 ) {
						$('#iinkainm').val($('#iinkaicd option:selected').text());
						$('#iinkaicdinsert').val($("#iinkaicd").val());
						$('#frmAdminCommittee').attr('target', '_blank');
						$('#frmAdminCommittee').attr('action', '../Committee/detail');
						$('#previewflg').val('0');
						form.submit();
					} else {
						$.confirm({
							...
							buttons: {
								OK: {
									action: function(){
										...
										$('#frmAdminCommittee').attr('target', '');
										$('#frmAdminCommittee').attr('action', '../AdminCommitteInfo/register');
										var data = new FormData();
										var serializedData = $("#frmAdminCommittee").serialize();
										 			...
										 			$.confirm({
														...
														buttons: {
															OK: {
																...
																keys: ['enter'],
																action: function(){
																	$('#menuFrm').attr('action', '../AdminCommitteInfo/edit');
																	$("#menuFrm").submit();
																}
															}
														}
