<!doctype html>
<title>出欠情報（会議）：ニアショアＩＴ協会 管理画面</title>
...
<?= $this->Html->script('admin/attendanceKaigi/list.js') ?>
<!-- ========== header ========== -->
...
<section class="main">
	<div class="container maxWidth1250">
		<main>
			...
			<h1 class="main-title">出欠情報（会議）</h1>
			<?php echo $this->Form->create('backtohome', ['id' => 'backtohome', 'url' => ['controller' => 'admin', 'action' => 'home']]); echo $this->Form->end();?>
			<?php echo $this->Form->create('adminAttendanceKaigi', ['id' => 'AdminAttendanceKaigi', 'name' => 'AdminAttendanceKaigi', 'url' => ['controller' => 'AdminAttendanceKaigi', 'action' => 'search']]);
			...
			<table style="width: 100%;">
					<tbody>
						...
						<tr>
							...
							<td class="tac">
								<?php	
									if(isset($attenInfo['kaigi']['total']) && $attenInfo['kaigi']['total'] != 0) {
										echo $attenInfo['kaigi']['total'];
									}
								?>
							</td>
							<?php foreach ($activityList as $evtKey => $kbunruinmvalue) { ?>
							<?php if ($Katsudoucheck=="") { ?>
							<td class="tac"><?php if(isset($attenInfo['kaigi'][$evtKey]) && $attenInfo['kaigi'][$evtKey] != 0) {
								echo $attenInfo['kaigi'][$evtKey]; } ?></td>
