<!doctype html>
<title>有益情報 編集：一般社団法人ニアショアＩＴ協会 管理画面</title>
...
<?= $this->Html->script('admin/beneficial/edit.js') ?>
	...
	<div class="container">
		<main>
			<ol class="breadcrumbs" style="display: inline-block;">
				<li style="display: inline-block;"><a style="cursor:pointer;" id="backpage_head" class="backpage_head">管理画面トップ</a></li><li style="display: inline-block;">有益情報 編集</li>
			</ol><!-- /.breadcrumbs -->
			<div class="f_right breadcrumbs"><a style="cursor:pointer;" class="menuBack">一覧に戻る</a></div>
			<h1 class="main-title">有益情報 編集</h1>
			<?php echo $this->Form->create('menu', ['id' => 'menuFrm', 'url' => ['controller' => 'AdminBeneficial', 'action' => 'search']]);
			...
			echo $this->Form->end(); ?>
			<?php echo $this->Form->create('adminBeneficialregister', ['enctype' => 'multipart/form-data','id' => 'adminBeneficialregister', 'url' => ['controller' => 'AdminBeneficial', 'action' => 'update']]);
			echo $this->Form->input('hdn_soushin', array('id' => 'hdn_soushin','name' => 'hdn_soushin','type' => 'hidden', 'value' => 0));
			echo $this->Form->input('id', array('type' => 'hidden','name'=>'id','id'=>'id', 'value' => $yuekishousai['0']['TYueki']['arno']));
			...
			echo $this->Form->input('yuekiDtFrm', array('id'=>'yuekiDtFrm','type' => 'hidden','value'=>$yuekiDtFrm,'name'=>'yuekiDtFrm','id'=>'yuekiDtFrm'));
			echo $this->Form->input('yuekiDtTo', array('id'=>'yuekiDtTo','type' => 'hidden','value'=>$yuekiDtTo,'name'=>'yuekiDtTo','id'=>'yuekiDtTo'));
			...
			<div class="form-area">
				<dl class="form-common">
					...
					<dd>
						<div class="error-list">
							<?php echo $this->Form->input('yuekidate', array('class'=>'datepicker','id'=>'yuekidate','label'=>false,'type'=>'text','name'=>'yuekidate','value'=> mb_substr(str_replace('-', '/', $yuekishousai['0']['TYueki']['jyohodt']), 0,10 ),'maxlength' => '10')); ?>
						</div>
				...
				<dl class="form-common">
					...
					<dd>
						<div class="error-list">
						<?php echo $this->Form->input('yuekitime', array('type' => 'text','id' => 'yuekitime', 'label' => false, 'class' => 'ime-ModeDisable timebox','name'=>'yuekitime','value'=> mb_substr(str_replace('-', '/', $yuekishousai['0']['TYueki']['jyohodt']), 11,5 ), 'onkeypress' => 'return checkTimeValue(event, id, "title");'));?>
						</div>
				...
				<dl class="form-common">
					...
					<dd>
						<div class="error-list">
							<?php echo $this->Form->input('kaiinnm', array('type'=>'text','name' => 'kaiinnm','label' => false, 'id' => 'kaiinnm', 'class'=> 'doublebyte', 'maxlength'=> '40','placeholder'=>'ここに会員名称を入力してください。','value' => $yuekishousai['0']['TYueki']['kaiinnm'])); ?>
						</div>
				...
				<dl class="form-common">
					...
					<dd>
						<div class="error-list">
							<?php echo $this->Form->input('title', array('type'=>'text','name' => 'title','label' => false, 'id' => 'title', 'maxlength'=> '100','placeholder'=>'ここにタイトルを入力してください。','value' => $yuekishousai['0']['TYueki']['title'])); ?>
						</div>
				...
				<dl class="form-common">
					...
					<dd>
						<div class="error-list"><?php  echo $this->Form->textarea('notes', array('name' => 'naiyo','label' => false,'maxlength'=> '1024','id' => 'naiyo', 'escape' => false,'style'=>'width:668px;resize: none;','class' => 'noTrimSpace','value' => $yuekishousai['0']['TYueki']['naiyo']));?></div>
					...
					<dt class="thum_title">
						<div class="thum_box">
							...
							<figure class="thum_02">
								...
								<?php }else{?>
									<img src="<?php echo $this->base."/AdminBeneficial/getSyasin/".$syasinData['syasin1']."/".$syasinKey;?>" id="thum01" style="max-height: 53px;max-width: 73px;width: auto!important;"/>
									<?php echo $this->Form->input('', array('type' => 'hidden','name'=>'image1','id'=>'image1','value'=> $this->base."/Beneficial/getSyasinImage/".$syasinData['syasin1']."/".$syasinKey )); ?>
								<?php } ?>
							</figure>
						</div>
					...
					<dt class="thum_title">
						<div class="thum_box">
							...
							<figure class="thum_02">
								...
								<?php }else{?>
									<img src="<?php echo $this->base."/AdminBeneficial/getSyasin/".$syasinData['syasin2']."/".$syasinKey;?>" id="thum02" style="max-height: 53px;max-width: 73px;width: auto!important;"/>
									<?php echo $this->Form->input('', array('type' => 'hidden','name'=>'image2','id'=>'image2','value'=> $this->base."/Beneficial/getSyasinImage/".$syasinData['syasin2']."/".$syasinKey )); ?>
								<?php }?>
							</figure>
						</div>
					...
					<dt class="thum_title">
						<div class="thum_box">
							...
							<figure class="thum_02">
								...
								<?php }else{?>
									<img src="<?php echo $this->base."/AdminBeneficial/getSyasin/".$syasinData['syasin3']."/".$syasinKey;?>" id="thum03" style="max-height: 53px;max-width: 73px;width: auto!important;"/>
									<?php echo $this->Form->input('', array('type' => 'hidden','name'=>'image3','id'=>'image3','value'=> $this->base."/Beneficial/getSyasinImage/".$syasinData['syasin3']."/".$syasinKey )); ?>
								<?php }?>
							</figure>
						</div>
						...
						<div id="path1" style="margin-bottom: 10px;margin-top: 10px;" class="breadcrumbs inline">
							<?php
								if (file_exists(WWW_APP_ROOT.$yuekiFilePath.$path1file)) {
									echo $this->Html->link($filename1[1],Router::url('/', true).WWW_APP_ROOT.$yuekiFilePath.$path1file,['target' => '_blank', 'download'=>$filename1[1]]); 
								} else { ?>
						...
						<div id="path2" style="margin-bottom: 10px;margin-top: 10px;" class="breadcrumbs inline">
							<?php
								if (file_exists(WWW_APP_ROOT.$yuekiFilePath.$path2file)) {
									echo $this->Html->link($filename2[1],Router::url('/', true).WWW_APP_ROOT.$yuekiFilePath.$path2file,['target' => '_blank', 'download'=>$filename2[1]]); 
								} else { ?>
						...
						<div id="path3" style="margin-bottom: 10px;margin-top: 10px;" class="breadcrumbs inline">
							<?php
								if (file_exists(WWW_APP_ROOT.$yuekiFilePath.$path3file)) {
									echo $this->Html->link($filename3[1],Router::url('/', true).WWW_APP_ROOT.$yuekiFilePath.$path3file,['target' => '_blank', 'download'=>$filename3[1]]); 
								} else { ?>
				...
				<dl class="form-common">
					...
					<dd>
						<div class="error-list">
							<?php echo $this->Form->input('sankourl', array('type'=>'text','name' => 'sankourl','label' => false, 'id' => 'sankourl', 'maxlength'=> '512','placeholder'=>'','value' => $yuekishousai['0']['TYueki']['sankourl'])); ?>
						</div>
					...
					<dd>
						...
						<?php if(!empty($kokai)): foreach($kokai as $kokaiinfo): ?>
							<li><input type="radio" name="koukaikbn" id="<?php echo "koukaikbn_r".$kokaiinfo['MKoukai']['koukaicd'];?>" 
									value="<?php echo $kokaiinfo['MKoukai']['koukaicd'];?>" <?php if ($kokaiinfo['MKoukai']['koukaicd'] == $yuekishousai['0']['TYueki']['koukaikbn']) { ?>
