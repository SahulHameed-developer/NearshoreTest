...
<footer>
	...
	echo $this->Form->end(); ?>
	<div class="pagetop">
		<?php echo $this->Html->image('common/pc/pagetop.png', array('alt' => 'ページトップへ戻る','class' => 'gototop','style' => 'cursor:pointer;display:none;'));?>
	</div><!-- /.pagetop -->
	<div class="pagebottom breadcrumbs">
		<?php echo $this->Html->image('common/pc/pagebottom.png', array('alt' => 'ページボットムへ戻る','class' => 'gotobottom','style' => 'cursor:pointer;display:none;'));?>
	</div><!-- /Page_Bottom -->
	...
	$(document).ready(function() {
		...
		$('input, textarea').blur(function () {
			...
			if ($(this).attr('class') != undefined) {
				arrTrimSpace = ($(this).attr('class')).split(' ');
			}
...
<script type="text/javascript">
	...
	function timerIncrement() {
		...
		var userIdletime = window.localStorage.getItem("userIdletime");
			...
			$.confirm({
				...
				buttons: {
					OK: {
						...
						keys: ['enter'],
						action: function(){
							window.location = "<?php echo $this->Html->url(array('controller' => 'Admin', 'action' => 'sessionTimeout')); ?>";
							adminTrig = 0;
						}
					}
				}
