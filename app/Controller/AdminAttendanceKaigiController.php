 ...
 *
 */
class AdminAttendanceKaigiController extends AppController {
	// helpers追加
	...
	public function memberlist() {
		...
		//活動分類マスタテーブル
		$this->set('kbunruinm', $this->Common->getActivityKaigi($this->MSosiki));
		...
		$this->set('count','0');
		// 画面の移動	
		$this->render('/Admin/AttendanceKaigi/list');
	}
	/* 表示順序処理
	 * 総回数（降順） ：  総回数（降順）、協会役職コード、会員名称、の順
	 */
	public function sortByTotalDESCOrder($first, $next) {
		// return $next['kaigi']['total'] - $first['kaigi']['total'];
		if($next['kaigi']['total'] < $first['kaigi']['total']) {
			return -1;
		} else if($next['kaigi']['total'] > $first['kaigi']['total']) {
			return 1;
		} else {
	...
	public function sortByTotalASCOrder($first, $next) {
		if($first['kaigi']['total'] < $next['kaigi']['total']) {
			return -1;
		} else if($first['kaigi']['total'] > $next['kaigi']['total']) {
			return 1;
		} else {
			return self::getInnerSort($first, $next);
		}
	...
	public function search() {
		...
		if (!empty ( $this->request->data )) {
				...
				$empDetails = $this->TKaiin->find ( 'all', array (
					...
					if ($cnt == 0) {
						...
						if(isset($this->request->data ['Katsudoucheck'])) {
							$activityList = self::getActivityField($this->TKatudo, $this->MSosiki, $st_date, $en_date, $this->request->data ['Katsudoucheck']);
						} else {
							$checked = null;
							$activityList = self::getActivityField($this->TKatudo, $this->MSosiki, $st_date, $en_date, $checked);
						}
						$i=0;
						foreach ($activityList as $activityListkey => $kbunruinmvalue) {
							if(empty($activityListkey)) {
								...
								$evnetConditions[] = array ('TKatudo.sosikicd' => $activityListkey);
								$evnetConditions[] = array ('TKatudo.bunruicd' => ConstantsComponent::$KAIGI_CD);
								if (! empty ( $st_date )) {
							...
							foreach ($activityList as $activityListkey => $kbunruinmvalue) {
								$fetchData = $this->TEntry->find ( 'all', array (
										...
										'conditions' => array('TEntry.kaiincd' => $empCode,
																'TKatudo.sosikicd' => $activityListkey,
								...
								$attendanceData[$empCode]['kaigi'][$activityListkey] = $fetchData[0][0]['cnt'];
							}
							$attendanceData[$empCode]['kaigi']['total'] = $tot;
							// 出席率
			...
			$this->set('kaiinmei', $kaiinmei);
			$this->set('katsudo', $katsudo);
			$this->set('kbunruinm', $this->Common->getActivityKaigi($this->MSosiki, $st_date, $en_date));
			if($Shussekiritsu == '0') {
			...
			$this->redirect ( ['controller' => 'AdminAttendanceKaigi','action' => 'memberlist' ] );
		}
	}
	/**
	 * 　画面名：出欠情報
	 * 　機能名：活動フィールドを取得する処理
	 */
	public function getActivityField($TKatudo, $MSosiki, $st_date, $en_date, $checked = null) {
		$activityList  = array();
		if($checked != null) {
			$conditions[] = "";
			if(COUNT($checked) > 1) {
				$conditions[] = array('AND' => array('MSosiki.sosikicd IN ' => $checked));
			} else {
				$conditions[] = array('AND' => array('MSosiki.sosikicd' => $checked[0]));
			}
			$conditions[] = array('AND' => array('fromdt <= ' => $this->Common->getSystemDate()));
			$conditions[] = array('AND' => array('todt >= ' => $this->Common->getSystemDate()));
			$activityList = $MSosiki->find('list', array(
			...
			$activityList = $TKatudo->find('list', array(
											'joins' => array (
												array ('table' => $MSosiki,
														...
														'type' => 'LEFT',
														'conditions' => array (
																			'TKatudo.sosikicd = MSosiki.sosikicd',
																			'TKatudo.bunruicd' => ConstantsComponent::$KAIGI_CD,
																			'MSosiki.fromdt <= IF(TKatudo.kousindt ="0000-00-00 00:00:00" OR TKatudo.kousindt IS NULL, DATE(TKatudo.tourokudt), DATE(TKatudo.kousindt))',
																			'MSosiki.todt >= IF(TKatudo.kousindt ="0000-00-00 00:00:00" OR TKatudo.kousindt IS NULL, DATE(TKatudo.tourokudt), DATE(TKatudo.kousindt))'))
											),
										'fields' => array('MSosiki.sosikicd','MSosiki.sosikinm'),
										'conditions'=> $conditions,
										'order'=>array('MSosiki.hyojino' => 'ASC')
									));
		}
		return $activityList;
	}
