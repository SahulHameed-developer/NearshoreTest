...
class PasswordChangeController extends AppController {
		...
		try {
			...
			if(!$result || $affectedRow == 0) {
				$systemError = array('logdt' => $this->Common->getSystemDateTime(), 
						...
						'errurl' => Router::url('/', true).'PasswordChange/saihakko' , 'errsyousai' => '会員情報の更新が異常終了'
				);
	...
	private function mailGeneration($columnValue) {
		...
		$message .= '<br/>'.Router::url('/', true).'PasswordChange/saihakko?CD='.$url."</p>";
		return $message;
	}
